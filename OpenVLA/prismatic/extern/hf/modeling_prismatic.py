...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        use_cache = bool(use_cache) and not self.training
        ...
        # === Handle Generation with Cache =>> requires `past_key_values` ===
        past_seen_tokens = get_cache_seq_length(past_key_values)
        if past_seen_tokens > 0:
            assert input_ids.shape[0] == 1, "Generation is only currently supported for batch size of 1!"
            assert labels is None, "Unexpected key `labels` provided during cached generation!"

            decode_input_ids = input_ids[:, -1:]
            # Generation tracks text length; this cache retains the original multimodal length.
            logical_length = get_cache_seq_length(past_key_values, logical=True)
            decode_cache_position = torch.arange(
                logical_length,
                logical_length + decode_input_ids.shape[1],
                dtype=torch.long,
                device=decode_input_ids.device,
            )
            language_model_output = self.language_model(
                input_ids=decode_input_ids,
                # Only SIFT-VLA caches can expand text masks into per-layer multimodal masks.
                attention_mask=attention_mask if isinstance(past_key_values, SIFTVLACache) else None,
                position_ids=getattr(past_key_values, "next_position_ids", None),
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs: str,
        ...
        """Use the image during prefill and the latest action token during cached decode."""
        ...
        if past_seen_tokens > 0:
            if input_ids is None:
                raise ValueError("Cached decoding requires input_ids.")
            model_inputs = {"input_ids": input_ids[:, -1:]}
            pixel_values = None
        else:
            # Use inputs_embeds only on the first generation step.
    ...
    def _reorder_cache(self, past_key_values, beam_idx) -> Any:
        reordered = self.language_model._reorder_cache(past_key_values, beam_idx)
        if isinstance(past_key_values, SIFTVLACache):
            return past_key_values.with_reordered_values(reordered, beam_idx)
        return reordered
...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action(
        ...
        if input_ids is None or input_ids.ndim != 2 or input_ids.shape[1] == 0:
            raise ValueError("predict_action requires nonempty input_ids with shape [batch, sequence].")
        ...
        if not torch.all(input_ids[:, -1] == 29871):
            input_ids = torch.cat(
                (input_ids, input_ids.new_full((input_ids.shape[0], 1), 29871)), dim=1
            )
            if attention_mask is not None:
