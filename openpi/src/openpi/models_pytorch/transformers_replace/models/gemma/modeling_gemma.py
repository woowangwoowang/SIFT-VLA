...
# Checked by the policy wrapper before allocating model weights.
OPENPI_BLOCK_ATTENTION_VERSION = 1
...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # HF may omit an ordinary causal mask for SDPA's is_causal fast
        # path. Materialize it before dispatch so the policy-aware backend
        # never interprets that omission as bidirectional attention. Explicit
        # pi0 prefix/action block masks pass through unchanged.
        if attention_mask is None and self.is_causal:
            ...
            query_positions = torch.arange(
                key_length - query_length, key_length, device=query_states.device
            )
            attention_mask = (key_positions[None, :] <= query_positions[:, None])[None, None]

        # pi0 uses bidirectional prefix/action blocks. Generic causal FA2
        # dispatch cannot represent this mask; use the policy-aware backend.
        from openpi.models_pytorch.attention import attention_forward

        attention_interface = attention_forward
